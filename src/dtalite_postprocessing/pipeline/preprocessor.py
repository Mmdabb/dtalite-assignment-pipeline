...
def link_performance_preprocess(network_dir, time_period_list, length_unit="mile", speed_unit="mph", developer_mode=0):
    ...
    combined = pd.concat([processed_by_period[period.lower()] for period in time_period_list if period.lower() in processed_by_period], axis=0)
